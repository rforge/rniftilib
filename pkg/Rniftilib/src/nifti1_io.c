                                     ...
                                     const int * blist, nifti_brick_list * NBL)
{
   ...
   if( !hname || !NBL ){
      REprintf(/*fprintf(stderr,*/"** nifti_image_read_bricks: bad params (%p,%p)\n",
              hname, (void *)NBL);
      return NULL;
   }

   if( blist && nbricks <= 0 ){
      REprintf(/*fprintf(stderr,*/"** nifti_image_read_bricks: bad nbricks, %d\n", nbricks);
      return NULL;
   }
...
static void update_nifti_image_for_brick_list( nifti_image * nim , int nbricks )
{
   ...
   if( g_opts.debug > 2 ){
      REprintf(/*fprintf(stderr,*/"+d updating image dimensions for %d bricks in list\n",
              nbricks);
      REprintf(/*fprintf(stderr,*/"   ndim = %d\n",nim->ndim);
      REprintf(/*fprintf(stderr,*/"   nx,ny,nz,nt,nu,nv,nw: (%d,%d,%d,%d,%d,%d,%d)\n",
   ...
   if( g_opts.debug > 2 ){
      REprintf(/*fprintf(stderr,*/"+d ndim = %d -> %d\n",nim->ndim, ndim);
      REprintf(/*fprintf(stderr,*/" --> (%d,%d,%d,%d,%d,%d,%d)\n",
              nim->nx, nim->ny, nim->nz, nim->nt, nim->nu, nim->nv, nim->nw);
   }
...
int nifti_update_dims_from_array( nifti_image * nim )
{
   ...
   if( !nim ){
      REprintf(/*fprintf(stderr,*/"** update_dims: missing nim\n");
      return 1;
   }
                             ...
                             const int * blist, nifti_brick_list * NBL )
{
   ...
   if( !nim || !NBL ){
      REprintf(/*fprintf(stderr,*/"** nifti_image_load_bricks, bad params (%p,%p)\n",
              (void *)nim, (void *)NBL);
      return -1;
   }
   ...
   if( blist && nbricks <= 0 ){
      if( g_opts.debug > 1 )
         REprintf(/*fprintf(stderr,*/"-d load_bricks: received blist with nbricks = %d,"
                        "ignoring blist\n", nbricks);
   ...
   if( !fp ){
      if( g_opts.debug > 0 )
         REprintf(/*fprintf(stderr,*/"** nifti_image_load_bricks, failed load_prep\n");
                                  ...
                                  nifti_brick_list * NBL, znzFile fp )
{
   ...
   if( test < 0 ){
      REprintf(/*fprintf(stderr,*/"** load bricks: ztell failed??\n");
      return -1;
   }
   fposn = oposn = test;
   ...
   if( !slist ){
      for( c = 0; c < NBL->nbricks; c++ ) {
         ...
         if( rv != NBL->bsize ){
            REprintf(/*fprintf(stderr,*/"** load bricks: cannot read brick %d from '%s'\n",
                    c, nim->iname ? nim->iname : nim->fname);
            return -1;
         }
      }
      if( g_opts.debug > 1 )
         REprintf(/*fprintf(stderr,*/"+d read %d default %u-byte bricks from file %s\n",
   ...
   if( !sindex ){
      REprintf(/*fprintf(stderr,*/"** load_NBL_bricks: missing index list\n");
      return -1;
   }
                               ...
                               nifti_brick_list * nbl)
{
   ...
   if( ! nbl->bricks ){
      REprintf(/*fprintf(stderr,*/"** NANM: failed to alloc %d void ptrs\n",nbricks);
      return -1;
   }
   ...
   for( c = 0; c < nbl->nbricks; c++ ){
      ...
      if( ! nbl->bricks[c] ){
         REprintf(/*fprintf(stderr,*/"** NANM: failed to alloc %u bytes for brick %d\n",
   ...
   if( g_opts.debug > 2 )
      REprintf(/*fprintf(stderr,*/"+d NANM: alloc'd %d bricks of %u bytes for NBL\n",
              nbl->nbricks, (unsigned int)nbl->bsize);
                           ...
                           int ** sindex)
{
   ...
   *sindex = (int *)malloc(nbricks * sizeof(int));
   ...
   if( !*slist || !*sindex ){
      REprintf(/*fprintf(stderr,*/"** NCS: failed to alloc %d ints for sorting\n",nbricks);
   ...
   if( g_opts.debug > 2 ){
      REprintf(/*fprintf(stderr,*/  "+d sorted indexing list:\n");
      REprintf(/*fprintf(stderr,*/  "  orig   : ");
      for( c1 = 0; c1 < nbricks; c1++ ) REprintf(/*fprintf(stderr,*/ "  %d",blist[c1]);
      REprintf(/*fprintf(stderr,*/"\n  new    : ");
      for( c1 = 0; c1 < nbricks; c1++ ) REprintf(/*fprintf(stderr,*/ "  %d",stmp[c1]);
      REprintf(/*fprintf(stderr,*/"\n  indices: ");
      for( c1 = 0; c1 < nbricks; c1++ ) REprintf(/*fprintf(stderr,*/ "  %d",itmp[c1]);
      REprintf("\n"); /*fputc('\n', stderr);*/
   }
   ...
   for( c1 = 0; c1 < nbricks-1; c1++ ){
       if( (stmp[c1] > stmp[c1+1]) || (blist[itmp[c1]] != stmp[c1]) ){
          REprintf(/*fprintf(stderr,*/"** sorting screw-up, way to go, rick!\n");
          free(stmp); free(itmp); *slist = NULL; *sindex = NULL;
          return -1;
       }
   }

   if( g_opts.debug > 2 ) REprintf(/*fprintf(stderr,*/"-d sorting is okay\n");
                           ...
                           const int * blist, int disp_error)
{
   ...
   if( !nim ){
      if( disp_error || g_opts.debug > 0 )
         REprintf(/*fprintf(stderr,*/"** valid_nifti_brick_list: missing nifti image\n");
      return 0;
   }

   if( nbricks <= 0 || !blist ){
      if( disp_error || g_opts.debug > 1 )
         REprintf(/*fprintf(stderr,*/"** valid_nifti_brick_list: no brick list to check\n");
      return 0;
   }

   if( nim->dim[0] < 3 ){
      if( disp_error || g_opts.debug > 1 )
         REprintf(/*fprintf(stderr,*/"** cannot read explict brick list from %d-D dataset\n",
                 nim->dim[0]);
      return 0;
   }
   ...
   if( nsubs <= 0 ){
      REprintf(/*fprintf(stderr,*/"** VNBL warning: bad dim list (%d,%d,%d,%d)\n",
                                 ...
                                 const nifti_brick_list *NBL)
{
   ...
   if( !nim || !NBL ) {
      if( g_opts.debug > 0 )
         REprintf(/*fprintf(stderr,*/"** nifti_NBL_matches_nim: NULL pointer(s)\n");
      return 0;
   }
   ...
   if( volbytes != NBL->bsize ) {
      if( g_opts.debug > 1 )
         REprintf(/*fprintf(stderr,*/"** NBL/nim mismatch, volbytes = %u, %u\n",
   ...
   if( nvols != NBL->nbricks ) {
      if( g_opts.debug > 1 )
         REprintf(/*fprintf(stderr,*/"** NBL/nim mismatch, nvols = %d, %d\n",
                 NBL->nbricks, nvols);
      errs++;
   }
   ...
   if( errs ) return 0;
      ...
      REprintf(/*fprintf(stderr,*/"-- nim/NBL agree: nvols = %d, nbytes = %u\n",
...
char *nifti_strdup(const char *str)
{
  ...
  else      REprintf(/*fprintf(stderr,*/"** nifti_strdup: failed to alloc %u bytes\n", 
...
void nifti_swap_Nbytes( size_t n , int siz , void *ar )  /* subsuming case */
{
   switch( siz ){
     ...
     default:    /* nifti_swap_bytes  ( n , siz, ar ) ; */
        REprintf(/*fprintf(stderr,*/"** NIfTI: cannot swap in %d byte blocks\n", siz);
        break ;
   }
   return ;
}
...
int nifti_is_complete_filename(const char* fname)
{
   ...
   if( fname == NULL || *fname == '\0' ){
      if ( g_opts.debug > 1 )
         REprintf(/*fprintf(stderr,*/"-- empty filename in nifti_validfilename()\n");
      return 0;
   }
   ...
   if ( ext == NULL ) { /*Invalid extension given */
      if ( g_opts.debug > 0 )
         REprintf(/*fprintf(stderr,*/"-- no nifti valid extension for filename '%s'\n", fname);
       return 0;
   }

   if ( ext && ext == fname ) {   /* then no filename prefix */
      if ( g_opts.debug > 0 )
         REprintf(/*fprintf(stderr,*/"-- no prefix for filename '%s'\n", fname);
      return 0;
   }
   return 1;
}
...
char * nifti_find_file_extension( const char * name )
{
   ...
   if( compare_strlist(extcopy, elist, 4) >= 0 ) {
      if( is_mixedcase(ext) ) {
         REprintf(/*fprintf(stderr,*/"** mixed case extension '%s' is not valid\n", ext);
         return NULL;
      }
      else return ext;
   }
   ...
   if( g_opts.debug > 1 )
      REprintf(/*fprintf(stderr,*/"** find_file_ext: failed for name '%s'\n", name);
...
char * nifti_findhdrname(const char* fname)
{
   ...
   if( !hdrname ){
      REprintf(/*fprintf(stderr,*/"** nifti_findhdrname: failed to alloc hdrname\n");
      free(basename);
      return NULL;
   }
...
char * nifti_findimgname(const char* fname , int nifti_type)
{
   ...
   if( !imgname ){
      REprintf(/*fprintf(stderr,*/"** nifti_findimgname: failed to alloc imgname\n");
      free(basename);
      return NULL;
   }
                         ...
                         int comp)
{
   ...
   if( !iname ){ REprintf(/*fprintf(stderr,*/"** small malloc failure!\n"); return NULL; }
   strcpy(iname, prefix);
   ...
   if( check && nifti_fileexists(iname) ){
      REprintf(/*fprintf(stderr,*/"** failure: header file '%s' already exists\n",iname);
      free(iname);
      return NULL;
   }

   if(g_opts.debug > 2) REprintf(/*fprintf(stderr,*/"+d made header filename '%s'\n", iname);
                         ...
                         int comp)
{
   ...
   /* check for existence failure */
   if( check && nifti_fileexists(iname) ){
      REprintf(/*fprintf(stderr,*/"** failure: image file '%s' already exists\n",iname);
      free(iname);
      return NULL;
   }

   if( g_opts.debug > 2 ) REprintf(/*fprintf(stderr,*/"+d made image filename '%s'\n",iname);
                         ...
                         int set_byte_order )
{
   ...
   if( !nim || !prefix ){
      REprintf(/*fprintf(stderr,*/"** nifti_set_filenames, bad params %p, %p\n",
              (void *)nim,prefix);
      return -1;
   }

   if( g_opts.debug > 1 )
      REprintf(/*fprintf(stderr,*/"+d modifying output filenames using prefix %s\n", prefix);
   ...
   if( g_opts.debug > 2 )
      REprintf(/*fprintf(stderr,*/"+d have new filenames %s and %s\n",nim->fname,nim->iname);
...
int nifti_type_and_names_match( nifti_image * nim, int show_warn )
{
   ...
   /* sanity checks */
   if( !nim ){
      if( show_warn ) REprintf(/*fprintf(stderr,*/"** %s: missing nifti_image\n", func);
      return -1;
   }
   if( !nim->fname ){
      if( show_warn ) REprintf(/*fprintf(stderr,*/"** %s: missing header filename\n", func);
      errs++;
   }
   if( !nim->iname ){
      if( show_warn ) REprintf(/*fprintf(stderr,*/"** %s: missing image filename\n", func);
      errs++;
   }
   if( !is_valid_nifti_type(nim->nifti_type) ){
      if( show_warn )
         REprintf(/*fprintf(stderr,*/"** %s: bad nifti_type %d\n", func, nim->nifti_type);
      errs++;
   }
   ...
   if( !ext_h ){
      if( show_warn )
         REprintf(/*fprintf(stderr,*/"-d missing NIFTI extension in header filename, %s\n",
                 nim->fname);
      errs++;
   }
   if( !ext_i ){
      if( show_warn )
         REprintf(/*fprintf(stderr,*/"-d missing NIFTI extension in image filename, %s\n",
                 nim->iname);
      errs++;
   }
            ...
            (nim->nifti_type == NIFTI_FTYPE_ANALYZE) )
   {
      if( fileext_n_compare(ext_h,".hdr",4) != 0 ){
         if( show_warn )
            REprintf(/*fprintf(stderr,*/"-d no '.hdr' extension, but NIFTI type is %d, %s\n",
                    nim->nifti_type, nim->fname);
         errs++;
      }
      if( fileext_n_compare(ext_i,".img",4) != 0 ){
         if( show_warn )
            REprintf(/*fprintf(stderr,*/"-d no '.img' extension, but NIFTI type is %d, %s\n",
...
int nifti_set_type_from_names( nifti_image * nim )
{
   ...
   if( !nim ){ REprintf(/*fprintf(stderr,*/"** NSTFN: no nifti_image\n");  return -1; }

   if( !nim->fname || !nim->iname ){
      REprintf(/*fprintf(stderr,*/"** NSTFN: missing filename(s) fname @ %p, iname @ %p\n",
              nim->fname, nim->iname);
      return -1;
   }
   ...
   if( ! nifti_validfilename      ( nim->fname ) ||
      ...
      REprintf(/*fprintf(stderr,*/"** NSTFN: invalid filename(s) fname='%s', iname='%s'\n",
              nim->fname, nim->iname);
      return -1;
   }

   if( g_opts.debug > 2 )
      REprintf(/*fprintf(stderr,*/"-d verify nifti_type from filenames: %d",nim->nifti_type);
   ...
   if( g_opts.debug > 2 ) REprintf(/*fprintf(stderr,*/" -> %d\n",nim->nifti_type);
   ...
   REprintf(/*fprintf(stderr,*/"** NSTFN: bad nifti_type %d, for '%s' and '%s'\n",
...
int is_nifti_file( const char *hname )
{
   ...
   if( tmpname == NULL ){
      if( g_opts.debug > 0 )
         REprintf(/*fprintf(stderr,*/"** no header file found for '%s'\n",hname);
      return -1;
   }
                                    ...
                                    const char * fname)
{
  ...
  if( !is_nifti || nhdr.qform_code <= 0 ){
    ...
    if( g_opts.debug > 1 ) REprintf(/*fprintf(stderr,*/"-d no qform provided\n");
  } else {
  ...
  if( !is_nifti || nhdr.sform_code <= 0 ){
    ...
    if( g_opts.debug > 1 ) REprintf(/*fprintf(stderr,*/"-d no sform provided\n");
...
nifti_1_header * nifti_read_header(const char * hname, int * swapped, int check)
{
   ...
   if( hfile == NULL ){
      ...
      REprintf(/*fprintf(stderr,*/"-d %s: found header filename '%s'\n",fname,hfile);
   ...
   if( bytes < (int)sizeof(nhdr) ){
      if( g_opts.debug > 0 ){
         ...
         REprintf(/*fprintf(stderr,*/"  - read %d of %d bytes\n",bytes, (int)sizeof(nhdr));
      }
      return NULL;
   }
   ...
   if( check && lswap < 0 ){
      ...
      if(g_opts.debug > 1) REprintf(/*fprintf(stderr,*/"-- swap failure, none applied\n");
   }
   ...
   if( ! hptr ){
      REprintf(/*fprintf(stderr,*/"** nifti_read_hdr: failed to alloc nifti_1_header\n");
      return NULL;
   }
...
int nifti_hdr_looks_good(const nifti_1_header * hdr)
{
   ...
   if( need_nhdr_swap(hdr->dim[0], hdr->sizeof_hdr) < 0 ){
      if( g_opts.debug > 0 )
         REprintf(/*fprintf(stderr,*/"** bad nhdr fields: dim0, sizeof_hdr = %d, %d\n",
   ...
   for( c = 1; c <= hdr->dim[0] && c <= 7; c++ )
      if( hdr->dim[c] <= 0 ){
         if( g_opts.debug > 0 )
            REprintf(/*fprintf(stderr,*/"** bad nhdr field: dim[%d] = %d\n",c,hdr->dim[c]);
         errs++;
      }
   ...
   if( is_nifti ){      /* NIFTI */
      ...
      if( ! nifti_datatype_is_valid(hdr->datatype, 1) ){
         if( g_opts.debug > 0 )
            REprintf(/*fprintf(stderr,*/"** bad NIFTI datatype in hdr, %d\n",hdr->datatype);
         errs++;
      }
      ...
      if( ! nifti_datatype_is_valid(hdr->datatype, 0) ){
         if( g_opts.debug > 0 )
           REprintf(/*fprintf(stderr,*/"** bad ANALYZE datatype in hdr, %d\n",hdr->datatype);
         errs++;
      }
   }
   ...
   if( g_opts.debug > 2 ) REprintf(/*fprintf(stderr,*/"-d nifti header looks good\n");
...
static int need_nhdr_swap( short dim0, int hdrsize )
{
   ...
   if( d0 != 0 ){     /* then use it for the check */
      ...
      if( g_opts.debug > 1 ){
         REprintf(/*fprintf(stderr,*/"** NIFTI: bad swapped d0 = %d, unswapped = ", d0);
         nifti_swap_2bytes(1, &d0);        /* swap? */
         REprintf(/*fprintf(stderr,*/"%d\n", d0);
      }
   ...
   if( g_opts.debug > 1 ){
      REprintf(/*fprintf(stderr,*/"** NIFTI: bad swapped hsize = %d, unswapped = ", hsize);
      nifti_swap_4bytes(1, &hsize);        /* swap? */
      REprintf(/*fprintf(stderr,*/"%d\n", hsize);
   }
...
nifti_image *nifti_image_read( const char *hname , int read_data )
{
   ...
   if( g_opts.debug > 1 ){
      REprintf(/*fprintf(stderr,*/"-d image_read from '%s', read_data = %d",hname,read_data);
#ifdef HAVE_LIBZ
      REprintf(/*fprintf(stderr,*/", HAVE_LIBZ = 1\n");
#else
      REprintf(/*fprintf(stderr,*/", HAVE_LIBZ = 0\n");
#endif
   }
   ...
   if( g_opts.debug > 3 ){
      REprintf(/*fprintf(stderr,*/"+d nifti_image_read(), have nifti image:\n");
                                     ...
                                     int read_data)
{
   ...
   if( g_opts.debug > 1 )
      REprintf(/*fprintf(stderr,*/"-d %s: have ASCII NIFTI file of size %d\n",fname,slen);

   if( slen > 65530 ) slen = 65530 ;
   sbuf = (char *)calloc(sizeof(char),slen+1) ;
   if( !sbuf ){
      REprintf(/*fprintf(stderr,*/"** %s: failed to alloc %d bytes for sbuf",lfunc,65530);
      free(fname);  znzclose(fp);  return NULL;
   }
   ...
   if( read_data && rv != 0 ){
      if( g_opts.debug > 1 )
         REprintf(/*fprintf(stderr,*/"-d failed image_load, free nifti image struct\n");
      free(nim);
      return NULL;
   }
...
static int nifti_read_extensions( nifti_image *nim, znzFile fp, int remain )
{
   ...
   if( !nim || znz_isnull(fp) ) {
      if( g_opts.debug > 0 )
         REprintf(/*fprintf(stderr,*/"** nifti_read_extensions: bad inputs (%p,%p)\n",
                 (void *)nim, (void *)fp);
      return -1;
   }
   ...
   if( (posn != sizeof(nifti_1_header)) &&
       (nim->nifti_type != NIFTI_FTYPE_ASCII) )
      REprintf(/*fprintf(stderr,*/"** WARNING: posn not header size (%d, %d)\n",
              posn, (int)sizeof(nifti_1_header));

   if( g_opts.debug > 2 )
      REprintf(/*fprintf(stderr,*/"-d nre: posn = %d, offset = %d, type = %d, remain = %d\n",
              posn, nim->iname_offset, nim->nifti_type, remain);

   if( remain < 16 ){
      if( g_opts.debug > 2 ){
         if( g_opts.skip_blank_ext )
            REprintf(/*fprintf(stderr,*/"-d no extender in '%s' is okay, as "
                           "skip_blank_ext is set\n",nim->fname);
         else
            REprintf(/*fprintf(stderr,*/"-d remain=%d, no space for extensions\n",remain);
      }
      return 0;
   }
   ...
   if( count < 4 ){
      if( g_opts.debug > 1 )
         REprintf(/*fprintf(stderr,*/"-d file '%s' is too short for an extender\n",
                 nim->fname);
      return 0;
   }
   ...
   if( extdr.extension[0] != 1 ){
      if( g_opts.debug > 2 )
         REprintf(/*fprintf(stderr,*/"-d extender[0] (%d) shows no extensions for '%s'\n",
   ...
   remain -= 4;
   if( g_opts.debug > 2 )
      REprintf(/*fprintf(stderr,*/"-d found valid 4-byte extender, remain = %d\n", remain);
   ...
   while (nifti_read_next_extension(&extn, nim, remain, fp) > 0)
   {
      if( nifti_add_exten_to_list(&extn, &Elist, count+1) < 0 ){
         if( g_opts.debug > 0 )
            REprintf(/*fprintf(stderr,*/"** failed adding ext %d to list\n", count);
         return -1;
      }

      /* we have a new extension */
      if( g_opts.debug > 1 ){
         REprintf(/*fprintf(stderr,*/"+d found extension #%d, code = 0x%x, size = %d\n",
                 count, extn.ecode, extn.esize);
         if( extn.ecode == NIFTI_ECODE_AFNI && g_opts.debug > 2 ) /* ~XML */
            REprintf(/*fprintf(stderr,*/"   AFNI extension: %.*s\n",
                    extn.esize-8,extn.edata);
         else if( extn.ecode == NIFTI_ECODE_COMMENT && g_opts.debug > 2 )
            REprintf(/*fprintf(stderr,*/"   COMMENT extension: %.*s\n",        /* TEXT */
                    extn.esize-8,extn.edata);
      }
      remain -= extn.esize;
      count++;
   ...
   if( g_opts.debug > 2 ) REprintf(/*fprintf(stderr,*/"+d found %d extension(s)\n", count);
                                    ...
                                    nifti1_extension ** list, int new_length )
{
   ...
   *list = (nifti1_extension *)malloc(new_length * sizeof(nifti1_extension));
   ...
   if( ! *list ){
      REprintf(/*fprintf(stderr,*/"** failed to alloc %d extension structs (%d bytes)\n",
   ...
   if( g_opts.debug > 2 )
      REprintf(/*fprintf(stderr,*/"+d allocated and appended extension #%d to list\n",
              new_length);
                                ...
                                int len, int ecode)
{
   ...
   if( !ext || !data || len < 0 ){
      REprintf(/*fprintf(stderr,*/"** fill_ext: bad params (%p,%p,%d)\n",
              (void *)ext, data, len);
      return -1;
   } else if( ! nifti_is_valid_ecode(ecode) ){
      REprintf(/*fprintf(stderr,*/"** fill_ext: invalid ecode %d\n", ecode);
      return -1;
   }
   ...
   if( !ext->edata ){
      REprintf(/*fprintf(stderr,*/"** NFE: failed to alloc %d bytes for extension\n",len);
      return -1;
   }
   ...
   if( g_opts.debug > 2 )
      REprintf(/*fprintf(stderr,*/"+d alloc %d bytes for ext len %d, ecode %d, esize %d\n",
                                      ...
                                      int remain, znzFile fp )
{
   ...
   if( remain < 16 ){
      if( g_opts.debug > 2 )
         REprintf(/*fprintf(stderr,*/"-d only %d bytes remain, so no extension\n", remain);
      return 0;
   }
   ...
   if( count != 2 ){
      if( g_opts.debug > 2 )
         REprintf(/*fprintf(stderr,*/"-d current extension read failed\n");
   ...
   if( swap ){
      if( g_opts.debug > 2 )
         REprintf(/*fprintf(stderr,*/"-d pre-swap exts: code %d, size %d\n", code, size);
   ...
   if( g_opts.debug > 2 )
      REprintf(/*fprintf(stderr,*/"-d potential extension: code %d, size %d\n", code, size);

   if( !nifti_check_extension(nim, size, code, remain) ){
      if( znzseek(fp, -8, SEEK_CUR) < 0 ){      /* back up past any read */
         REprintf(/*fprintf(stderr,*/"** failure to back out of extension read!\n");
         return -1;
      }
      return 0;
   }
   ...
   if( !nex->edata ){
      REprintf(/*fprintf(stderr,*/"** failed to allocate %d bytes for extension\n",size);
      return -1;
   }
   ...
   if( count < size ){
      if( g_opts.debug > 0 )
         REprintf(/*fprintf(stderr,*/"-d read only %d (of %d) bytes for extension\n",
                 count, size);
      free(nex->edata);
      nex->edata = NULL;
      return -1;
   ...
   /* success! */
   if( g_opts.debug > 2 )
      REprintf(/*fprintf(stderr,*/"+d successfully read extension, code %d, size %d\n",
              nex->ecode, nex->esize);
...
int valid_nifti_extensions(const nifti_image * nim)
{
   ...
   if( nim->num_ext <= 0 || nim->ext_list == NULL ){
      if( g_opts.debug > 2 ) REprintf(/*fprintf(stderr,*/"-d empty extension list\n");
      return 0;
   }
   ...
   for ( c = 0; c < nim->num_ext; c++ ){
      if( ! nifti_is_valid_ecode(ext->ecode) ) {
         if( g_opts.debug > 1 )
            REprintf(/*fprintf(stderr,*/"-d ext %d, invalid code %d\n", c, ext->ecode);
         errs++;
      }

      if( ext->esize <= 0 ){
         if( g_opts.debug > 1 )
            REprintf(/*fprintf(stderr,*/"-d ext %d, bad size = %d\n", c, ext->esize);
         errs++;
      } else if( ext->esize & 0xf ){
         if( g_opts.debug > 1 )
            REprintf(/*fprintf(stderr,*/"-d ext %d, size %d not multiple of 16\n",
                    c, ext->esize);
         errs++;
      }

      if( ext->edata == NULL ){
         if( g_opts.debug > 1 ) REprintf(/*fprintf(stderr,*/"-d ext %d, missing data\n", c);
         errs++;
      }
   ...
   if( errs > 0 ){
      if( g_opts.debug > 0 )
         REprintf(/*fprintf(stderr,*/"-d had %d extension errors, none will be written\n",
                 errs);
      return 0;
   }
...
static int nifti_check_extension(nifti_image *nim, int size, int code, int rem)
{
   ...
   if( ! nifti_is_valid_ecode(code) ) {
      if( g_opts.debug > 2 )
         REprintf(/*fprintf(stderr,*/"-d invalid extension code %d\n",code);
      return 0;
   }

   if( size < 16 ){
      if( g_opts.debug > 2 )
         REprintf(/*fprintf(stderr,*/"-d ext size %d, no extension\n",size);
      return 0;
   }

   if( size > rem ){
      if( g_opts.debug > 2 )
         REprintf(/*fprintf(stderr,*/"-d ext size %d, space %d, no extension\n", size, rem);
      return 0;
   }

   if( size & 0xf ){
      if( g_opts.debug > 2 )
         REprintf(/*fprintf(stderr,*/"-d nifti extension size %d not multiple of 16\n",size);
      return 0;
   }

   if( nim->nifti_type == NIFTI_FTYPE_ASCII && size > LNI_MAX_NIA_EXT_LEN ){
      if( g_opts.debug > 2 )
         REprintf(/*fprintf(stderr,*/"-d NVE, bad nifti_type 3 size %d\n", size);
      return 0;
   }
...
static znzFile nifti_image_load_prep( nifti_image *nim )
{
   ...
   {
      if ( g_opts.debug > 0 ){
         if( !nim ) REprintf(/*fprintf(stderr,*/"** ERROR: N_image_load: no nifti image\n");
         else REprintf(/*fprintf(stderr,*/"** ERROR: N_image_load: bad params (%p,%d,%u)\n",
   ...
   if( tmpimgname == NULL ){
      if( g_opts.debug > 0 )
         REprintf(/*fprintf(stderr,*/"** no image file found for '%s'\n",nim->iname);
      return NULL;
   }
   ...
   if( znzseek(fp , (long)ioff , SEEK_SET) < 0 ){
      REprintf(/*fprintf(stderr,*/"** could not seek to offset %u in file '%s'\n",
...
int nifti_image_load( nifti_image *nim )
{
   ...
   if( fp == NULL ){
      if( g_opts.debug > 0 )
         REprintf(/*fprintf(stderr,*/"** nifti_image_load, failed load_prep\n");
      return -1;
   }
   ...
   if( nim->data == NULL )
   {
     ...
     if( nim->data == NULL ){
        if( g_opts.debug > 0 )
           REprintf(/*fprintf(stderr,*/"** failed to alloc %d bytes for image data\n",
                   (int)ntot);
        znzclose(fp);
        return -1;
     }
                                ...
                                nifti_image *nim)
{
  ...
  if( dataptr == NULL ){
     if( g_opts.debug > 0 )
        REprintf(/*fprintf(stderr,*/"** ERROR: nifti_read_buffer: NULL dataptr\n");
     return -1;
  }
  ...
  if( ii < ntot ){ 
    if( g_opts.debug > 0 )
       REprintf(/*fprintf(stderr,*/"++ WARNING: nifti_read_buffer(%s):\n"
               "   data bytes needed = %u\n"
               "   data bytes input  = %u\n"
  ...
  if( g_opts.debug > 2 )
    REprintf(/*fprintf(stderr,*/"+d nifti_read_buffer: read %u bytes\n", (unsigned)ii);
  ...
  if( nim->swapsize > 1 && nim->byteorder != nifti_short_order() ) {
    if( g_opts.debug > 1 )
       REprintf(/*fprintf(stderr,*/"+d nifti_read_buffer: swapping data bytes...\n");
...
int nifti_free_extensions( nifti_image *nim )
{
   ...
   /* or if it is inconsistent, warn the user (if we are not in quiet mode) */
      ...
      REprintf(/*fprintf(stderr,*/"** warning: nifti extension num/ptr mismatch (%d,%p)\n",
              nim->num_ext, (void *)nim->ext_list);

   if( g_opts.debug > 2 )
      REprintf(/*fprintf(stderr,*/"+d free'd %d extension(s)\n", nim->num_ext);
...
size_t nifti_write_buffer(znzFile fp, const void *buffer, size_t numbytes)
{
   ...
   size_t ss;
   if (znz_isnull(fp)){
      REprintf(/*fprintf(stderr,*/"** ERROR: nifti_write_buffer: null file pointer\n");
      return 0;
   }
                         ...
                         const nifti_brick_list * NBL)
{
   ...
   if( !NBL ){ /* just write one buffer and get out of here */
      if( nim->data == NULL ){
         REprintf(/*fprintf(stderr,*/"** NWAD: no image data to write\n");
         return -1;
      }
      ...
      if( g_opts.debug > 1 )
         REprintf(/*fprintf(stderr,*/"+d wrote single image of %u bytes\n", (unsigned)ss);
   } else {
      if( ! NBL->bricks || NBL->nbricks <= 0 || NBL->bsize <= 0 ){
         REprintf(/*fprintf(stderr,*/"** NWAD: no brick data to write (%p,%d,%u)\n",
      ...
      if( g_opts.debug > 1 )
         REprintf(/*fprintf(stderr,*/"+d wrote image of %d brick(s), each of %u bytes\n",
                 NBL->nbricks, (unsigned int)NBL->bsize);
   }
...
static int nifti_write_extensions(znzFile fp, nifti_image *nim)
{
   ...
   if( znz_isnull(fp) || !nim || nim->num_ext < 0 ){
      if( g_opts.debug > 0 )
         REprintf(/*fprintf(stderr,*/"** nifti_write_extensions, bad params\n");
      return -1;
   }
   ...
   if( g_opts.skip_blank_ext && (nim->num_ext == 0 || ! nim->ext_list ) ){
      if( g_opts.debug > 1 )
         REprintf(/*fprintf(stderr,*/"-d no exts and skip_blank_ext set, "
   ...
   if( nifti_write_buffer(fp, extdr, 4) != 4 ){
      REprintf(/*fprintf(stderr,*/"** failed to write extender\n");
      return -1;
   }
   ...
   for ( c = 0; c < nim->num_ext; c++ ){
      ...
      if( !ok ){
         REprintf(/*fprintf(stderr,*/"** failed while writing extension #%d\n",c);
         return -1;
      } else if ( g_opts.debug > 2 )
         REprintf(/*fprintf(stderr,*/"+d wrote extension %d of %d bytes\n", c, size);

      list++;
   }

   if( g_opts.debug > 1 )
      REprintf(/*fprintf(stderr,*/"+d wrote out %d extension(s)\n", nim->num_ext);
...
nifti_1_header * nifti_make_new_header(const int arg_dims[], int arg_dtype)
{
   ...
   if( dim[0] < 1 || dim[0] > 7 ) {
      REprintf(/*fprintf(stderr,*/"** nifti_simple_hdr_with_dims: bad dim[0]=%d\n",dim[0]);
      dim = default_dims;
   } else {
      for( c = 1; c <= dim[0]; c++ )
         if( dim[c] < 1 )
   ...
   if( ! nifti_is_valid_datatype(dtype) ) {
      REprintf(/*fprintf(stderr,*/"** nifti_simple_hdr_with_dims: bad dtype %d\n",dtype);
      dtype = DT_FLOAT32;
   }
   ...
   if( g_opts.debug > 1 )
      REprintf(/*fprintf(stderr,*/"+d nifti_make_new_header, dim[0] = %d, datatype = %d\n",
              dim[0], dtype);

   nhdr = (nifti_1_header *)calloc(1,sizeof(nifti_1_header));
   if( !nhdr ){
      REprintf(/*fprintf(stderr,*/"** nifti_make_new_header: failed to alloc hdr\n");
      return NULL;
   }
...
nifti_image * nifti_make_new_nim(const int dims[], int datatype, int data_fill)
{
   ...
   if( !nim ){
      REprintf(/*fprintf(stderr,*/"** NMNN: nifti_convert_nhdr2nim failure\n");
      return NULL;
   }

   if( g_opts.debug > 1 )
      REprintf(/*fprintf(stderr,*/"+d nifti_make_new_nim, data_fill = %d\n",data_fill);
   ...
   if( data_fill ) {
      ...
      if( !nim->data ) {
         REprintf(/*fprintf(stderr,*/"** NMNN: failed to alloc %u bytes for data\n",
...
int nifti_copy_extensions(nifti_image * nim_dest, const nifti_image * nim_src)
{
   ...
   if( nim_dest->num_ext > 0 || nim_dest->ext_list != NULL ){
      REprintf(/*fprintf(stderr,*/"** will not copy extensions over existing ones\n");
      return -1;
   }

   if( g_opts.debug > 1 )
      REprintf(/*fprintf(stderr,*/"+d duplicating %d extension(s)\n", nim_src->num_ext);
   ...
   if( !nim_dest->ext_list ){
      REprintf(/*fprintf(stderr,*/"** failed to allocate %d nifti1_extension structs\n",
              nim_src->num_ext);
      return -1;
   }
   ...
   for( c = 0; c < nim_src->num_ext; c++ ){
      ...
      if( g_opts.debug > 2 )
         REprintf(/*fprintf(stderr,*/"+d dup'ing ext #%d of size %d (from size %d)\n",
                 c, size, old_size);
      /* data length is size-8, as esize includes space for esize and ecode */
      data = (char *)calloc(size-8,sizeof(char));      /* maybe size > old */
      if( !data ){
         REprintf(/*fprintf(stderr,*/"** failed to alloc %d bytes for extention\n", size);
...
int nifti_extension_size(nifti_image *nim)
{
   ...
   if( g_opts.debug > 2 ) REprintf(/*fprintf(stderr,*/"-d ext sizes:");

   for ( c = 0; c < nim->num_ext; c++ ){
      size += nim->ext_list[c].esize;
      if( g_opts.debug > 2 ) REprintf(/*fprintf(stderr,*/"  %d",nim->ext_list[c].esize);
   }

   if( g_opts.debug > 2 ) REprintf(/*fprintf(stderr,*/" (total = %d)\n",size);
   ...
   switch( nim->nifti_type ){
     ...
     case NIFTI_FTYPE_NIFTI1_1:
       ...
       if( nim->iname_offset != offset ){
          if( g_opts.debug > 1 )
             REprintf(/*fprintf(stderr,*/"+d changing offset from %d to %d\n",
               ...
               const char * opts, znzFile imgfile, const nifti_brick_list * NBL)
{
   ...
   if( g_opts.debug > 1 ){
      REprintf(/*fprintf(stderr,*/"-d writing nifti file '%s'...\n", nim->fname);
      if( g_opts.debug > 2 )
         REprintf(/*fprintf(stderr,*/"-d nifti type %d, offset %d\n",
   ...
   if( ! znz_isnull(imgfile) && nim->nifti_type == NIFTI_FTYPE_NIFTI1_1 ){
      if( g_opts.debug > 2 ) REprintf(/*fprintf(stderr,*/"+d using passed file for hdr\n");
      fp = imgfile;
   }
   else {
      if( g_opts.debug > 2 )
         REprintf(/*fprintf(stderr,*/"+d opening output file %s [%s]\n",nim->fname,opts);
   ...
   if( ! write_data && ! leave_open ){
      if( g_opts.debug > 2 ) REprintf(/*fprintf(stderr,*/"-d header is all we want: done\n");
      znzclose(fp); return(fp);
   }
   ...
   if( nim->nifti_type != NIFTI_FTYPE_NIFTI1_1 ){ /* get a new file pointer */
      ...
      if( ! znz_isnull(imgfile) ){
         if(g_opts.debug > 2) REprintf(/*fprintf(stderr,*/"+d using passed file for img\n");
         fp = imgfile;
      }
      else {
         if( g_opts.debug > 2 )
            REprintf(/*fprintf(stderr,*/"+d opening img file '%s'\n", nim->iname);
                              ...
                              const char *opts, int write_data, int leave_open)
{
   ...
   if( ! hstr ){ REprintf(/*fprintf(stderr,*/"** failed image_to_ascii()\n"); return NULL; }
                                                                                
   fp = znzopen( nim->fname , opts , nifti_is_gzfile(nim->fname) ) ;
   if( znz_isnull(fp) ){
      free(hstr);
      REprintf(/*fprintf(stderr,*/"** failed to open '%s' for ascii write\n",nim->fname);
      return fp;
   }
...
void nifti_image_write( nifti_image *nim )
{
   ...
   if( fp ){
      if( g_opts.debug > 2 ) REprintf(/*fprintf(stderr,*/"-d niw: done with znzFile\n");
      free(fp);
   }
   if( g_opts.debug > 1 ) REprintf(/*fprintf(stderr,*/"-d nifti_image_write: done\n");
}
...
void nifti_image_write_bricks( nifti_image *nim, const nifti_brick_list * NBL )
{
   ...
   if( fp ){
      if( g_opts.debug > 2 ) REprintf(/*fprintf(stderr,*/"-d niwb: done with znzFile\n");
      free(fp);
   }
   if( g_opts.debug > 1 ) REprintf(/*fprintf(stderr,*/"-d niwb: done writing bricks\n");
}
...
nifti_image * nifti_copy_nim_info(const nifti_image * src)
{
  ...
  if( !dest ){
     REprintf(/*fprintf(stderr,*/"** NCNI: failed to alloc nifti_image\n");
     return NULL;
  }
...
static char *escapize_string( const char * str )
{
   ...
   if( !out ){
      REprintf(/*fprintf(stderr,*/"** escapize_string: failed to alloc %d bytes\n",lout);
      return NULL;
   }
...
char *nifti_image_to_ascii( const nifti_image *nim )
{
   ...
   if( !buf ){
      REprintf(/*fprintf(stderr,*/"** NITA: failed to alloc %d bytes\n",65534);
      return NULL;
   }
   ...
   if( !buf ) REprintf(/*fprintf(stderr,*/"** NITA: failed to realloc %d bytes\n",nbuf+1);
   return buf ;
}
...
nifti_image *nifti_image_from_ascii( const char *str, int * bytes_read )
{
   ...
   nim = (nifti_image *)calloc( 1 , sizeof(nifti_image) ) ;
   if( !nim ){
      REprintf(/*fprintf(stderr,*/"** NIFA: failed to alloc nifti_image\n");
      return NULL;
   }
...
int nifti_nim_is_valid(nifti_image * nim, int complain)
{
   ...
   if( !nim ){
      REprintf(/*fprintf(stderr,*/"** is_valid_nim: nim is NULL\n");
      return 0;
   }

   if( g_opts.debug > 2 ) REprintf(/*fprintf(stderr,*/"-d nim_is_valid check...\n");
...
int nifti_nim_has_valid_dims(nifti_image * nim, int complain)
{
   ...
   if( nim->dim[0] <= 0 || nim->dim[0] > 7 ){
      errs++;
      if( complain )
         REprintf(/*fprintf(stderr,*/"** NVd: dim[0] (%d) out of range [1,7]\n",nim->dim[0]);
      return 0;
   }
   ...
   if( nim->ndim != nim->dim[0] ){
      errs++;
      if( ! complain ) return 0;
      REprintf(/*fprintf(stderr,*/"** NVd: ndim != dim[0] (%d,%d)\n",nim->ndim,nim->dim[0]);
   }
   ...
   if( ( (nim->dim[0] >= 1) && (nim->dim[1] != nim->nx) ) ||
      ...
      errs++;
      if( !complain ) return 0;
      REprintf(/*fprintf(stderr,*/"** NVd mismatch: dims    = %d,%d,%d,%d,%d,%d,%d\n"
                     "                 nxyz... = %d,%d,%d,%d,%d,%d,%d\n",
   ...
   if( g_opts.debug > 2 ){
      REprintf(/*fprintf(stderr,*/"-d check dim[%d] =", nim->dim[0]);
      for( c = 0; c < 7; c++ ) REprintf(/*fprintf(stderr,*/" %d", nim->dim[c]);
      REprintf("\n"); /*fputc('\n', stderr);*/
   }
   ...
   for( c = 1; c <= nim->dim[0]; c++ ){
      if( nim->dim[c] > 0)
         ...
         if( !complain ) return 0;
         REprintf(/*fprintf(stderr,*/"** NVd: dim[%d] (=%d) <= 0\n",c, nim->dim[c]);
         errs++;
      }
   }
   if( prod != nim->nvox ){
      if( ! complain ) return 0;
      REprintf(/*fprintf(stderr,*/"** NVd: nvox does not match %d-dim product (%u, %u)\n",
   ...
   if( g_opts.debug > 1 )
      for( c = nim->dim[0]+1; c <= 7; c++ )
         if( nim->dim[c] != 0 && nim->dim[c] != 1 )
            REprintf(/*fprintf(stderr,*/"** NVd warning: dim[%d] = %d, but ndim = %d\n",
                    c, nim->dim[c], nim->dim[0]);

   if( g_opts.debug > 2 )
      REprintf(/*fprintf(stderr,*/"-d nim_has_valid_dims check, errs = %d\n", errs);
                                ...
                                void ** data )
{
   ...
   if( !nim || !dims || !data ){
      REprintf(/*fprintf(stderr,*/"** nifti_RCI: bad params %p, %p, %p\n",
              (void *)nim, (void *)dims, (void *)data);
      return -1;
   }

   if( g_opts.debug > 2 ){
      REprintf(/*fprintf(stderr,*/"-d read_collapsed_image:\n        dims =");
      for(c = 0; c < 8; c++) REprintf(/*fprintf(stderr,*/" %3d", dims[c]);
      REprintf(/*fprintf(stderr,*/"\n   nim->dims =");
      for(c = 0; c < 8; c++) REprintf(/*fprintf(stderr,*/" %3d", nim->dim[c]);
      REprintf("\n"); /*fputc('\n', stderr);*/
   }

   /** - verify that dim[] makes sense */
   if( ! nifti_nim_is_valid(nim, g_opts.debug > 0) ){
      REprintf(/*fprintf(stderr,*/"** invalid nim (file is '%s')\n", nim->fname );
      return -1;
   }
   ...
   for( c = 1; c <= nim->dim[0]; c++ ){
      if( dims[c] >= nim->dim[c] ){
         REprintf(/*fprintf(stderr,*/"** nifti_RCI: dims[%d] >= nim->dim[%d] (%d,%d)\n",
   ...
   if( g_opts.debug > 1 )
      REprintf(/*fprintf(stderr,*/"+d read %d bytes of collapsed image from %s\n",
              bytes, nim->fname);
  ...
  */
  ...
  for(i = 0; i < nim->ndim; i++)
    ...
    if(start_index[i]  + region_size[i] > image_size[i])
      ...
      if(g_opts.debug > 1)
        {
        REprintf(/*fprintf(stderr,*/"region doesn't fit within image size\n");
        }
      return -1;
      }
    }
  ...
  if(*data == 0)
    ...
    if(g_opts.debug > 1)
      {
      REprintf(/*fprintf(stderr,*/"allocation of %d bytes failed\n",total_alloc_size);
      return -1;
      }
    }
          ...
          for(m = si[2]; m < (si[2] + rs[2]); m++)
            ...
            for(n = si[1]; n < (si[1] + rs[1]); n++)
              ...
              if(nread != read_amount)
                ...
                if(g_opts.debug > 1)
                  {
                  REprintf(/*fprintf(stderr,*/"read of %d bytes failed\n",read_amount);
                  return -1;
                  }
                }
              bytes += nread;
      ...
      int nprods, const int dims[], char * data, znzFile fp, size_t base_offset)
{
   ...
   if( nprods <= 0 ){
      REprintf(/*fprintf(stderr,*/"** rci_read_data, bad prods, %d\n", nprods);
      return -1;
   }
   ...
   if( nprods == 1 ){
      ...
      if( *pivots != 0 ){
         REprintf(/*fprintf(stderr,*/"** rciRD: final pivot == %d!\n", *pivots);
         return -1;
      }
      ...
      if( nread != bytes ){
         REprintf(/*fprintf(stderr,*/"** rciRD: read only %u of %u bytes from '%s'\n",
                 (unsigned)nread, (unsigned)bytes, nim->fname);
         return -1;
      } else if( g_opts.debug > 3 )
         REprintf(/*fprintf(stderr,*/"+d successful read of %u bytes at offset %u\n",
   ...
   for( c = 0; c < prods[0]; c++ ){
      ...
      if( g_opts.debug > 3 )
         REprintf(/*fprintf(stderr,*/"-d reading %u bytes, foff %u + %u, doff %u\n",
...
static int rci_alloc_mem(void ** data, int prods[8], int nprods, int nbyper )
{
   ...
   if( nbyper < 0 || nprods < 1 || nprods > 8 ){
      REprintf(/*fprintf(stderr,*/"** rci_am: bad params, %d, %d\n", nbyper, nprods);
      return -1;
   }
   ...
   if( ! *data ){   /* then allocate what is needed */
      if( g_opts.debug > 1 )
         REprintf(/*fprintf(stderr,*/"+d alloc %d (= %d x %d) bytes for collapsed image\n",
                 size, size/nbyper, nbyper);

      *data = malloc(size);   /* actually allocate the memory */
      if( ! *data ){
         REprintf(/*fprintf(stderr,*/"** rci_am: failed to alloc %d bytes for data\n", size);
         return -1;
      }
   } else if( g_opts.debug > 1 )
      REprintf(/*fprintf(stderr,*/"-d rci_am: *data already set, need %d (%d x %d) bytes\n",
              size, size/nbyper, nbyper);
                                              ...
                                              int prods[], int * nprods )
{
   ...
   *nprods = len;
   ...
   if( g_opts.debug > 2 ){
      REprintf(/*fprintf(stderr,*/"+d pivot list created, pivots :");
      for(index = 0; index < len; index++) REprintf(/*fprintf(stderr,*/" %d", pivots[index]);
      REprintf(/*fprintf(stderr,*/", prods :");
      for(index = 0; index < len; index++) REprintf(/*fprintf(stderr,*/" %d", prods[index]);
      REprintf("\n"); /*fputc('\n',stderr);*/
   }
...
int * nifti_get_intlist( int nvals , const char * str )
{
   ...
   if( !subv ) {
      REprintf(/*fprintf(stderr,*/"** nifti_get_intlist: failed alloc of 2 ints\n");
      return NULL;
   }
   subv[0] = nout = 0 ;
   ...
   if( g_opts.debug > 1 )
      REprintf(/*fprintf(stderr,*/"-d making int_list (vals = %d) from '%s'\n", nvals, str);
   ...
   while( ipos < slen && !ISEND(str[ipos]) ){
     ...
     while( isspace((int) str[ipos]) ) ipos++ ;   /* skip blanks */
      ...
      if( str[ipos] == '$' ){  /* special case */
         ...
         if( ibot < 0 ){
           REprintf(/*fprintf(stderr,*/"** ERROR: list index %d is out of range 0..%d\n",
                   ibot,nvals-1) ;
         ...
         nused = (cpt-(str+ipos)) ;
         if( ibot == 0 && nused == 0 ){
           REprintf(/*fprintf(stderr,*/"** ERROR: list syntax error '%s'\n",str+ipos) ;
           free(subv) ; return NULL ;
         }
         ipos += nused ;
      }
      ...
      if( str[ipos] == ',' || ISEND(str[ipos]) ){
         ...
         if( !subv ) {
            REprintf(/*fprintf(stderr,*/"** nifti_get_intlist: failed realloc of %d ints\n",
                    nout+1);
            return NULL;
         }
         subv[0]    = nout ;
      ...
      if( str[ipos] == '-' ){
         ...
         REprintf(/*fprintf(stderr,*/"** ERROR: index list syntax is bad: '%s'\n",
                 str+ipos) ;
         free(subv) ; return NULL ;
      }
      ...
      if( str[ipos] == '$' ){  /* special case */
         ...
         if( itop < 0 ){
           REprintf(/*fprintf(stderr,*/"** ERROR: index %d is out of range 0..%d\n",
                   itop,nvals-1) ;
           free(subv) ; return NULL ;
         }
         if( itop >= nvals ){
         ...
         nused = (cpt-(str+ipos)) ;
         if( itop == 0 && nused == 0 ){
           REprintf(/*fprintf(stderr,*/"** ERROR: index list syntax error '%s'\n",str+ipos) ;
           free(subv) ; return NULL ;
         }
         ipos += nused ;
      }
      ...
      if( str[ipos] == '(' ){  /* decode an integer */
         ...
         if( istep == 0 ){
           REprintf(/*fprintf(stderr,*/"** ERROR: index loop step is 0!\n") ;
           free(subv) ; return NULL ;
         }
         nused = (cpt-(str+ipos)) ;
         ipos += nused ;
         ...
         if( (ibot-itop)*istep > 0 ){
           REprintf(/*fprintf(stderr,*/"** WARNING: index list '%d..%d(%d)' means nothing\n",
                   ibot,itop,istep ) ;
         }
      }
   ...
   if( g_opts.debug > 1 ) {
      REprintf(/*fprintf(stderr,*/"+d int_list (vals = %d): ", subv[0]);
      for( ii = 1; ii <= subv[0]; ii++ ) REprintf(/*fprintf(stderr,*/"%d ", subv[ii]);
      REprintf("\n"); /*fputc('\n',stderr);*/
   }
...
int nifti_test_datatype_sizes(int verb)
{
    ...
    for( c = 0; c < tablen; c++ )
    {
                ...
                ssize != nifti_type_list[c].swapsize )
        {
            if( verb || g_opts.debug > 2 )
                REprintf(/*fprintf(stderr,*/ "** type mismatch: %s, %d, %d, %d : %d, %d\n",
    ...
    if( errs )
        REprintf(/*fprintf(stderr,*/"** nifti_test_datatype_sizes: found %d errors\n",errs);
    else if( verb || g_opts.debug > 1 )
        REprintf(/*fprintf(stderr,*/"-- nifti_test_datatype_sizes: all OK\n");
