...
char *Rnifti_attributes[] =
{
	...
	"sizeof_hdr",	    /* 20 */
	"datatype",         /* 21 */
	"scl_slope",        /* 22 nifti1: Data scaling: slope.  analyze 7.5: float funused1;      */
    "scl_inter",        /* 23 nifti1: Data scaling: offset. analyze 7.5: float funused2;      */
	NULL
};
	...
	{	
		...
		switch(iIndex)
		{
			...
			case 6: /*fname*/
			  ...
			  warning("Can not set this attribute directly! Please use the nifti_set_filenames function.\n"); break;
			case 7: /*iname*/
			...
			case 19: /* nifti_type 19 */
		  	    switch(pnim->nifti_type)
			    {
			  	  case NIFTI_FTYPE_ANALYZE:
			  	    return Rnifti_pchar_SEXP("NIFTI_FTYPE_ANALYZE"); break;
			  	  case NIFTI_FTYPE_NIFTI1_1:
			  	    return Rnifti_pchar_SEXP("NIFTI_FTYPE_NIFTI1_1"); break;
			  	  case NIFTI_FTYPE_NIFTI1_2:
			  	    return Rnifti_pchar_SEXP("NIFTI_FTYPE_NIFTI1_2"); break;
			  	  case NIFTI_FTYPE_ASCII:
			  	    return Rnifti_pchar_SEXP("NIFTI_FTYPE_ASCII"); break;
			  	  default:
			  	    return Rnifti_pchar_SEXP("NIFTI_FTYPE_UNKNOWN"); break;
			    }
			    break;
			case 20: /* sizeof_hdr 20 */
				{
			      struct nifti_1_header hdr =
			      nifti_convert_nim2nhdr(pnim);
...
SEXP Rnifti_image_getpixel(SEXP nim, SEXP sexp_x, SEXP sexp_y, SEXP sexp_z, SEXP sexp_t)
{
	SEXP ret_val=R_NilValue;
	nifti_image *pnim=SEXP2NIFTI(nim);
			  ...
			  for(iIndex[3]=0;iIndex[3]<LENGTH(coord[3]);++iIndex[3])
			    for(iIndex[2]=0;iIndex[2]<LENGTH(coord[2]);++iIndex[2])
			      for(iIndex[1]=0;iIndex[1]<LENGTH(coord[1]);++iIndex[1])
			        ...
			        {
			          iSOffset=iCoord[0][iIndex[0]]+iCoord[1][iIndex[1]]*pnim->dim[1]+iCoord[2][iIndex[2]]*pnim->dim[1]*pnim->dim[2]+iCoord[3][iIndex[3]]*pnim->dim[1]*pnim->dim[2]*pnim->dim[3];			          
...
SEXP Rnifti_image_printinfo(SEXP nim)
{
	...
	if(pnim!=NULL)
	{
		...
		Rprintf("(freq_dim; phase_dim; slice_dim) = (%d; %d; %d)\n",
		...
		switch(pnim->datatype)
	    {
			...
			case  DT_BINARY:                /*  1      binary (1 bit/voxel)         */
				Rprintf("nifti_image data type: 1 binary (1 bit/voxel)\n"); break;
			case  DT_UNSIGNED_CHAR:        /*  2      unsigned char (8 bits/voxel) */
				Rprintf("nifti_image data type: 2 unsigned char (8 bits/voxel)\n"); break;
			case  DT_SIGNED_SHORT:         /*  4      signed short (16 bits/voxel) */
				Rprintf("nifti_image data type: 4 signed short (16 bits/voxel)\n"); break;
			case  DT_SIGNED_INT:           /*  8      signed int (32 bits/voxel)   */
				Rprintf("nifti_image data type: 8 signed int (32 bits/voxel)\n"); break;
			case  DT_FLOAT:                /* 16      float (32 bits/voxel)        */
				Rprintf("nifti_image data type: 16 float (32 bits/voxel)\n"); break;
			case  DT_COMPLEX:              /* 32      complex (64 bits/voxel)      */
				Rprintf("nifti_image data type: 32 complex (64 bits/voxel)\n"); break;
			case  DT_DOUBLE:               /* 64      double (64 bits/voxel)       */
				Rprintf("nifti_image data type: 64 double (64 bits/voxel) \n"); break;
			case  DT_RGB:                  /*128      RGB triple (24 bits/voxel)   */
				Rprintf("nifti_image data type: 128 RGB triple (24 bits/voxel)\n"); break;
			
			/*------------------- new codes for NIFTI ---*/
			case  DT_INT8:                 /* 256      signed char (8 bits)         */
				Rprintf("nifti_image data type: 256 signed char (8 bits)\n"); break;
			case  DT_UINT16:               /* 512      unsigned short (16 bits)     */
				Rprintf("nifti_image data type: 512 unsigned short (16 bits)\n"); break;
			case  DT_UINT32:               /* 768      unsigned int (32 bits)       */
				Rprintf("nifti_image data type: 768 unsigned int (32 bits)\n"); break;
			case  DT_INT64:                /*1024      long long (64 bits)          */
				Rprintf("nifti_image data type: 1024 long long (64 bits)\n"); break;
			case  DT_UINT64:               /*1280      unsigned long long (64 bits) */
				Rprintf("nifti_image data type: 1280 unsigned long long (64 bits)\n"); break;
			case  DT_FLOAT128:             /*1536      long double (128 bits)       */
				Rprintf("nifti_image data type: 1536 long double (128 bits)\n"); break;
			case  DT_COMPLEX128:           /*1792      double pair (128 bits)       */
				Rprintf("nifti_image data type: 1792 double pair (128 bits)\n"); break;
			case  DT_COMPLEX256:           /*2048      long double pair (256 bits)  */
				Rprintf("nifti_image data type: 2048 long double pair (256 bits)\n"); break;
			default:              		   /*         what it says, dude           */
				Rprintf("nifti_image data type: %u what it says, dude\n",pnim->datatype); break;
	    }
	}
...
SEXP Rnifti_image_setpixel(SEXP nim, SEXP sexp_x, SEXP sexp_y, SEXP sexp_z, SEXP sexp_t, SEXP value)
{
	SEXP ret_val=R_NilValue;
	nifti_image *pnim=SEXP2NIFTI(nim);
	...
	if(pnim!=NULL)
	{
		...
		{	*/
			switch(pnim->datatype)
			{
				// uchar
				case DT_UNSIGNED_CHAR:
//				  ((unsigned char*)pnim->data)[x+y*pnim->dim[1]+z*pnim->dim[1]*pnim->dim[2]+t*pnim->dim[1]*pnim->dim[2]*pnim->dim[3]]=(unsigned char)NUMERIC_POINTER(value)[0];
					for(iIndex[3]=0;iIndex[3]<LENGTH(coord[3]);++iIndex[3])
			    		for(iIndex[2]=0;iIndex[2]<LENGTH(coord[2]);++iIndex[2])
			      			for(iIndex[1]=0;iIndex[1]<LENGTH(coord[1]);++iIndex[1])
			        			for(iIndex[0]=0;iIndex[0]<LENGTH(coord[0]);++iIndex[0])
			        			{
						          iTOffset=iCoord[0][iIndex[0]]+iCoord[1][iIndex[1]]*pnim->dim[1]+iCoord[2][iIndex[2]]*pnim->dim[1]*pnim->dim[2]+iCoord[3][iIndex[3]]*pnim->dim[1]*pnim->dim[2]*pnim->dim[3];			          
				...
				// int
				case DT_SIGNED_INT:
	//			  ((int*)pnim->data)[x+y*pnim->dim[1]+z*pnim->dim[1]*pnim->dim[2]+t*pnim->dim[1]*pnim->dim[2]*pnim->dim[3]]=(int)NUMERIC_POINTER(value)[0];
				  for(iIndex[3]=0;iIndex[3]<LENGTH(coord[3]);++iIndex[3])
			    		for(iIndex[2]=0;iIndex[2]<LENGTH(coord[2]);++iIndex[2])
			      			for(iIndex[1]=0;iIndex[1]<LENGTH(coord[1]);++iIndex[1])
			        			for(iIndex[0]=0;iIndex[0]<LENGTH(coord[0]);++iIndex[0])
				...
				// float
				case DT_FLOAT:
				  //((float*)pnim->data)[x+y*pnim->dim[1]+z*pnim->dim[1]*pnim->dim[2]+t*pnim->dim[1]*pnim->dim[2]*pnim->dim[3]]=(float)NUMERIC_POINTER(value)[0];
				  for(iIndex[3]=0;iIndex[3]<LENGTH(coord[3]);++iIndex[3])
			    		for(iIndex[2]=0;iIndex[2]<LENGTH(coord[2]);++iIndex[2])
			      			for(iIndex[1]=0;iIndex[1]<LENGTH(coord[1]);++iIndex[1])
			        			for(iIndex[0]=0;iIndex[0]<LENGTH(coord[0]);++iIndex[0])
				...
				// double
				case DT_DOUBLE:
				  //((double*)pnim->data)[x+y*pnim->dim[1]+z*pnim->dim[1]*pnim->dim[2]+t*pnim->dim[1]*pnim->dim[2]*pnim->dim[3]]=(double)NUMERIC_POINTER(value)[0];
				  	for(iIndex[3]=0;iIndex[3]<LENGTH(coord[3]);++iIndex[3])
			    		for(iIndex[2]=0;iIndex[2]<LENGTH(coord[2]);++iIndex[2])
			      			for(iIndex[1]=0;iIndex[1]<LENGTH(coord[1]);++iIndex[1])
			        			for(iIndex[0]=0;iIndex[0]<LENGTH(coord[0]);++iIndex[0])
